...
function focusOnSelect<T extends Item>(store: Tabs<T>, itemState: ListItem<T>): Behavior {
	return createWatchBehavior((node) => {
		if (isKeySelected(store.selected, getKey(itemState.value))) setFocus(node, true);
	});
}
	...
	function list(node: HTMLElement) {
		...
		const destroy = applyBehaviors(node, [
			...
			createWatchBehavior(() => {
				...
				console.log(selectedIndex, state.items, state.value);
